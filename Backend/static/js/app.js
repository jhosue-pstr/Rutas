...
async function cargarEstadisticasGenerales() {
    try {
        ...
        document.getElementById('total-usuarios').textContent = data.total_usuarios?.toLocaleString() || '0';
        document.getElementById('total-buses-favoritos').textContent = data.total_buses_favoritos?.toLocaleString() || '0';
        document.getElementById('total-lugares-favoritos').textContent = data.total_lugares_favoritos?.toLocaleString() || '0';
...
async function cargarBusesFavoritos() {
    ...
    container.innerHTML = '<div class="loading">Cargando datos de buses favoritos</div>';

    try {
        const response = await fetch(`${API_BASE}/api/admin/estadisticas/buses-favoritos`);
        ...
        if (busesData.length > 0) {
            const busesConDetalles = await Promise.all(
                busesData.map(async (bus) => {
                    try {
                        const busResponse = await fetch(`${API_BASE}/api/admin/buses/${bus.id_bus || bus.IdBus}`);
                        ...
                        console.warn(`⚠️ No se pudieron cargar los detalles del bus ${bus.id_bus}`);
                        return {
                            ...bus,
                            numero: 'N/A',
                            placa: 'N/A',
            ...
            container.innerHTML = '<div class="loading">No hay datos de buses favoritos disponibles</div>';
        }
    } catch (error) {
        console.error('❌ Error:', error);
        mostrarError(container, 'No se pudieron cargar los datos de buses favoritos');
    }
}
            ...
            <tbody>
                ${buses.map((bus, index) => `
                    <tr>
                        ...
                        <td><span class="badge">${(bus.total_favoritos || bus.total || 0).toLocaleString()} favoritos</span></td>
                    </tr>
                `).join('')}
            </tbody>
        </table>
...
async function cargarLugaresFavoritos() {
    ...
    container.innerHTML = '<div class="loading">Cargando datos de lugares favoritos</div>';

    try {
        const response = await fetch(`${API_BASE}/api/admin/estadisticas/lugares-favoritos`);
        ...
        console.error('❌ Error:', error);
        mostrarError(container, 'No se pudieron cargar los datos de lugares favoritos');
    }
}
            ...
            <tbody>
                ${lugares.map((lugar, index) => `
                    <tr>
                        ...
                        <td><span class="badge">${(lugar.total || 1).toLocaleString()} registros</span></td>
                    </tr>
                `).join('')}
            </tbody>
        </table>
...
async function cargarUsuariosPorFecha() {
    ...
    container.innerHTML = '<div class="loading">Cargando datos de usuarios...</div>';
    ...
    try {
        ...
        try {
            const response = await fetch(`${API_BASE}/api/admin/estadisticas/usuarios-por-fecha?dias=${dias}`);
            if (!response.ok) throw new Error(`Error HTTP: ${response.status}`);
            data = await response.json();
        } catch (error) {
            console.warn('❌ Endpoint principal falló, usando fallback:', error);
            // Usar endpoint de fallback
            const fallbackResponse = await fetch(`${API_BASE}/api/admin/estadisticas/usuarios-fallback`);
        ...
        if (data.usuarios_por_fecha && data.usuarios_por_fecha.length > 0) {
            ...
            container.innerHTML = `
                <div class="debug-info">
                    Período: ${dias === 0 ? 'Todo el historial' : `Últimos ${dias} días`} | 
                    Total de usuarios en sistema: <strong>${data.estadisticas_generales?.total_usuarios || 0}</strong>
...
console.log('🎯 Dashboard cargado. Usa debugEndpoints() en la consola para diagnóstico.');